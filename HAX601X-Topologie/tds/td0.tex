	...
	\begin{td-sol} \,\\
		\begin{enumerate}
			...
			\begin{enumerate}[label=$(\roman*)$]
				...
				\item On a $x=y\Longrightarrow d_{FE}(x, y)=0$ par définition. Si $d_{FE}(x, y) = 0$ alors on a $x=y$ ou $d(x, D)+d(y, D) = 0$. Comme une distance est positive, cela implique $d(x, D)=d(y, D)=0$, soit que $x=y$
